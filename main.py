...
app = Flask(__name__)
app.secret_key = "super-secret-key-broken-nadeem-multiuser-isolated"
...
def verify_code():
    ...
    async def _verify(client, phone, code, password):
        ...
        avatar_url_path = ""
        try:
            photo_path = os.path.join(CACHE_DIR, f"{phone.replace('+', '')}.jpg")
            downloaded = await client.download_profile_photo(me, file=photo_path)
            if downloaded:
                avatar_url_path = f"/cache_images/{phone.replace('+', '')}.jpg"
        except Exception as e:
...
def get_groups():
    ...
    sessions = load_sessions()
    if not phone or phone not in sessions:
        ...
        async for dialog in client.iter_dialogs():
            if dialog.is_group or dialog.is_channel:
                ...
                try:
                    ...
                    photo_path = os.path.join(CACHE_DIR, f"group_{group_id}.jpg")
                    downloaded = await client.download_profile_photo(entity, file=photo_path)
                    if downloaded:
                        avatar_url = f"/cache_images/group_{group_id}.jpg"
                except:
                    pass
                groups.append({
                    "id": group_id,
...
# ------------------ Background Sender Worker (with its own loop) ------------------
def sender_worker(task_id, phone, target, speed_sec, messages, hater, last_hater):
    if phone not in active_sending_tasks:
        active_sending_tasks[phone] = []
    if task_id not in active_sending_tasks[phone]:
        active_sending_tasks[phone].append(task_id)
    ...
    while phone in active_sending_tasks and task_id in active_sending_tasks[phone]:
        try:
            loop.run_until_complete(_send_loop(task_id, phone, loop))
        except Exception as e:
            logging.error(f"Sender worker crashed: {e}. Restarting in 5s...")
            time.sleep(5)

    if phone in active_sending_tasks and task_id in active_sending_tasks[phone]:
        active_sending_tasks[phone].remove(task_id)
    if task_id in task_params:
        del task_params[task_id]
    loop.close()

async def _send_loop(task_id, phone, loop):
    sessions = load_sessions()
    ...
    client = None
    try:
        client = TelegramClient(StringSession(data["session_string"]), data["api_id"], data["api_hash"], loop=loop)
        await client.connect()
        ...
        while phone in active_sending_tasks and task_id in active_sending_tasks[phone]:
            try:
                ...
                try:
                    ...
                    logging.error(f"Failed to get entity for target {target}: {e}")
                    await asyncio.sleep(5)
                    continue

                current_hater = params.get('hater', '')
                current_speed = params.get('speed', 2)
                current_last = params.get('last_hater', '')
                current_msgs = params.get('messages', [])
                idx = params.get('idx', 0)

                if current_msgs:
                    base_msg = current_msgs[idx % len(current_msgs)].strip()
                    msg = f"{current_hater} {base_msg} {current_last}".strip()
                    task_params[task_id]['idx'] = idx + 1
                else:
                    msg = f"{current_hater} AUTOMATION ENGINE ONLINE {current_last}".strip()

                if msg:
                    await client.send_message(entity, msg)
                task_params[task_id]['last_send_time'] = int(time.time())
                await asyncio.sleep(current_speed)
            except FloodWaitError as e:
                await asyncio.sleep(e.seconds)
            except (ConnectionError, asyncio.TimeoutError, OSError) as conn_err:
                logging.error(f"Connection error: {conn_err}. Reconnecting...")
                break
            except Exception as e:
                logging.error(f"Unknown error in send loop: {e}")
                await asyncio.sleep(3)
    except Exception as e:
        logging.error(f"Error in _send_loop: {e}")
        raise
    finally:
        if client:
            try:
...
def auto_resume_all_tasks():
    print("🔄 COLD BOOT: INIT AUTOMATIC SYSTEM TASK RESUME ENGINE...")
    all_tasks = load_tasks()
    count = 0
    for phone, t_list in all_tasks.items():
        for t in t_list:
            task_id = t["task_id"]
            target = t["target"]
            hater = t.get("hater", "")
            speed = float(t.get("speed", 2))
            last_hater = t.get("last_hater", "")
            messages = t.get("messages", [])

            thread = threading.Thread(
                target=sender_worker,
                args=(task_id, phone, target, speed, messages, hater, last_hater),
                daemon=True
            )
            thread.start()
            count += 1
    print(f"✅ ENGINE SUCCESS: {count} ACTIVE LOADER TASKS RESUMED SAFELY IN THE BACKGROUND!")
...
def start_sending():
    ...
    if not phone or not target or not owner:
    ...
    task_id = f"task_{int(time.time())}_{target}"
    ...
    thread = threading.Thread(target=sender_worker, args=(task_id, phone, target, speed, lines, hater, last_hater), daemon=True)
    thread.start()
    return jsonify({"success": True})
...
def update_task():
    ...
    new_speed = float(request.form.get('speed', 2))
    new_last_hater = request.form.get('last_hater', '')
    new_target = request.form.get('target', '').strip()
    file = request.files.get('file')

    if task_id in task_params:
        task_params[task_id]['hater'] = new_hater
        task_params[task_id]['speed'] = new_speed
        task_params[task_id]['last_hater'] = new_last_hater
        if new_target:
            task_params[task_id]['target'] = new_target
        if file and file.filename:
            try:
                content = file.read().decode('utf-8', errors='ignore')
                new_lines = [line.strip() for line in content.split('\n') if line.strip()]
                if new_lines:
                    task_params[task_id]['messages'] = new_lines
                    task_params[task_id]['idx'] = 0
            except:
                pass
    else:
    ...
    all_tasks = load_tasks()
    if phone in all_tasks:
        for t in all_tasks[phone]:
...
def delete_task():
    ...
    all_tasks = load_tasks()
    if phone in all_tasks:
        all_tasks[phone] = [t for t in all_tasks[phone] if t["task_id"] != task_id]
        save_tasks(all_tasks)
...
def list_sessions():
    ...
    filtered_sessions = {p: v for p, v in sessions.items() if v.get('owner') == owner}
    filtered_tasks = {}

    for phone, t_list in tasks.items():
        user_tasks = [t for t in t_list if t.get('owner') == owner]
        if user_tasks:
            filtered_tasks[phone] = []
            for ut in user_tasks:
                task_id = ut["task_id"]
                last_send = None
                if task_id in task_params and 'last_send_time' in task_params[task_id]:
                    last_send = task_params[task_id]['last_send_time']
                filtered_tasks[phone].append({
                    "task_id": task_id,
                    "target": ut["target"],
                    "hater": ut["hater"],
                    "speed": ut["speed"],
                    "last_hater": ut["last_hater"],
                    "messages_count": len(ut["messages"]),
                    "start_time": ut.get("start_time", int(time.time())),
                    "last_send_time": last_send
                })

    return jsonify({"sessions": filtered_sessions, "tasks": filtered_tasks})
...
def list_all_sessions():
    ...
    for phone, sess in sessions.items():
        owner = sess.get('owner', 'unknown')
        if owner not in result:
            result[owner] = {'sessions': {}, 'tasks': {}}
        result[owner]['sessions'][phone] = sess
        if phone in tasks:
            result[owner]['tasks'][phone] = []
            for t in tasks[phone]:
                task_id = t["task_id"]
                last_send = None
                if task_id in task_params and 'last_send_time' in task_params[task_id]:
                    last_send = task_params[task_id]['last_send_time']
                result[owner]['tasks'][phone].append({
                    "task_id": task_id,
                    "target": t["target"],
                    "hater": t["hater"],
                    "speed": t["speed"],
                    "last_hater": t["last_hater"],
                    "messages_count": len(t["messages"]),
                    "start_time": t.get("start_time", int(time.time())),
                    "last_send_time": last_send
                })
    return jsonify(result)
...
if __name__ == "__main__":
    ...
    try:
        ...
        app.run(host="0.0.0.0", port=20148, debug=False, threaded=True)
